#!/usr/bin/env python3
"""
Debug script that exactly replicates the app's flow to identify the issue.
"""
...
def debug_app_exact_flow():
    """Exactly replicate the app's flow"""
    print("=== Debug: App Exact Flow ===")
    
    # Simulate the app's file loading (using load_uploaded_file)
    test_csv = os.path.join(project_root, 'tests', 'data', 'test.csv')
    
    # Load data exactly like the app does (but use load_file since we have a file path)
    print(f"Loading data using app's load_file...")
    ...
    json_str = json.dumps(user_json, indent=2)
    ...
    output_path = "debug_app_exact_flow.pptx"
    ...
    generate_presentation(
        ...
        audience_defs=user_json  # Pass original with __groups__
    )
    ...
    expected_group_charts = len(user_json['__groups__']) * len(raw_audience_data)
